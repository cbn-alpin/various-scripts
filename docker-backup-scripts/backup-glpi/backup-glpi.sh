...
# Année : 2025
# Script de sauvegarde de l’instance GLPI (glpi)
...
repertoire_sauvegarde="/home/admin/docker/glpi/backups"
date_du_jour=$(date +%F)
...
echo "Conteneur applicatif : ${conteneur_site}"
...
echo "Copie des fichiers GLPI (config, data, glpi)..."
for chemin in config data glpi; do
    docker cp "${conteneur_site}:/app/${chemin}" "${dossier_temporaire}/${chemin}"
done
echo ""

echo "Archivage des fichiers applicatifs..."
if ! tar czf "${dossier_cible}/${nom_sauvegarde}.files.tar.gz" -C "${dossier_temporaire}" .; then
    echo "Erreur : Archivage échoué."
    exit 1
fi
echo "Archive créée : ${nom_sauvegarde}.files.tar.gz"
echo ""
