...
# Année : 2025
# Script de sauvegarde de l’instance Shortener (shortener)
...
nom_sauvegarde="shortener"

# répertoire de travail
repertoire_sauvegarde="/home/admin/docker/shortener/backups"
date_du_jour=$(date +%F)
...
# conteneurs utilisés
conteneur_bdd="shortener-database"
...
echo "Dump SQL compressé terminé : ${nom_sauvegarde}.dump.sql.gz"
echo ""

# copie des fichiers locaux montés
mkdir -p "${dossier_temporaire}/local"
echo "Copie des fichiers locaux montés..."
cp -r ./mariadb/initdb.d "${dossier_temporaire}/local/initdb.d"
cp -r ./shlink/config "${dossier_temporaire}/local/config"
echo ""
