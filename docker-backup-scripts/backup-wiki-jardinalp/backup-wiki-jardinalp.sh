...
# Année : 2025
# Script de sauvegarde de l’instance YesWiki (wiki-jardinalp)
...
nom_sauvegarde="wiki-jardinalp"

# répertoire de travail
repertoire_sauvegarde="/home/admin/docker/wiki-jardinalp/backups"
date_du_jour=$(date +%F)
...
conteneur_site="wiki-jardinalp-php"

# image contenant mariadb-dump
image_dump="mariadb:11.1.2-jammy"
port_bdd=3306
...
nom_base=$(docker exec "${conteneur_bdd}" printenv MARIADB_DATABASE)
...
echo "Copie des fichiers du site YesWiki (/var/www/html)..."
...
# archivage des dossiers locaux importants
echo "Archivage des dossiers locaux (mariadb/, nginx/, yeswiki/)..."
for dossier in mariadb nginx yeswiki; do
    if [ -d "./${dossier}" ]; then
        cp -a "./${dossier}" "${dossier_temporaire}/"
    fi
done

if ! tar czf "${dossier_cible}/${nom_sauvegarde}.localdirs.tar.gz" -C "${dossier_temporaire}" mariadb nginx yeswiki; then
    echo "Erreur : Archivage local échoué."
    exit 1
fi
echo "Archive créée : ${nom_sauvegarde}.localdirs.tar.gz"
echo ""
